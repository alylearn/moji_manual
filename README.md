...
⚠️ 以上是单平台产品的做法。如果是 App + Web 双平台产品，导航结构和图片路径不能直接套用这一条，请参考上方「多平台产品扩展规范」。
...
当前已做基础移动端适配（断点 768px），经排查侧边栏抽屉化、卡片网格、场景卡片、图片弹窗等主要布局均使用了自适应写法（grid auto-fit / flex-wrap / vw-vh），核心场景基本可用。待优化：
- 极窄屏（< 360px）部分长文字溢出，未逐一验证
