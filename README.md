...
打开 `index.html`，搜索 `id="block-XXX"`，比如要给词条详情加图就搜 `id="block-citiao"`。
...
- **v28**（2026.06）：CSS 外置、Kori 截图入库、点击缩略图弹大图、目录按产品分子文件夹、图条改至文字下方横向排列（110px）；辞書全量截图入库（95张）；辞書文字更新（搜索概述、词条详情、收藏主页、背词设置、学习列表等13处）；新增更多计划、收藏→背词section
...
每个产品介绍段末尾标注「当前版本 vX.X.X」，和App实际版本保持一致。
...
**更新时机：** 产品发版后，如果说明书中涉及的功能有变动，同步更新对应版本号。如果只是bug修复版本、说明书内容无变化，可以不更新。
...
如果只更新了文字内容（没改图、没改样式），可以只上传 `index.html` 一个文件。
